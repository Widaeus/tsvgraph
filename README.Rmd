...
`tsvgraph` can be installed directly from GitHub using the `devtools` package. If you do not have `devtools` installed, you can install it using the command `install.packages("devtools")`. Then, install `tsvgraph` using:
...
It accepts a tsv file containing graph data generated from PimSOFT using above prerequisites. It can thereafter be exported to a data frame and incorporated to a plot.
